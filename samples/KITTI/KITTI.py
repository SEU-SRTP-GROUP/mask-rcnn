...
class KITTIDataset(utils.Dataset):
    ...
    def load_mask(self, image_id):
       ...
       Returns:
        ...
        for i  in range(len(info["rectBoxs"])-2,-1,-1):
...
def detect_and_color_splash(model, image_path=None, video_path=None):
    ...
    class_names = ['Background', 'Pedestrian', 'Car', 'Cyclist']
    # Image or video?
    if image_path:
...
#为frustum-pointnet专门写的一个检测的函数

def rgb_Detect_For_FPointnet(model,imageSetPath=None,split ="testing"):
    '''
        input: imageSetPath: 照片的文件夹的路径
        output: 检测 imageSetPath下所有照片，将所有的检测结果写入一个文本文件，放在当前文件夹下。
    '''
    def isImage(image):
    ...
    assert(imageSetPath)
    rgbDetections=[]
    images = os.listdir(imageSetPath)
	#进行排序因为 os.listdir 读出来的是乱序的
    images.sort()
    #遍历文件夹
    for imagePath in images:
        if not isImage(imagePath):   #如果不是图片的话
            continue;
        filePath = os.path.join("dataset/KITTI/object",split,"image_2",imagePath)
        imagePath = os.path.join(imageSetPath,imagePath)  #拼接绝对路径
        #读取文件，并检测
        image = skimage.io.imread(imagePath)
        results = model.detect([image], verbose=1)
        result = results[0]
        ...
        for i in range(0,len(class_ids)):
            # 格式是 "filename classID prob x1 y1 x2 y2"  
            rgbDection="{} {} {} {} {} {} {}\n".format(filePath,class_ids[i],scores[i],rois[i][1],rois[i][0],rois[i][3],rois[i][2])               
            rgbDetections.append(rgbDection)

    #写入文件
    rgb_detection = os.path.join(imageSetPath,"rgb_detection.txt")
    with open(rgb_detection,"w") as rgb_detection:
            rgb_detection.writelines(rgbDetections)
############################################################
#  Training
############################################################
...
if __name__ == '__main__':
    ...
    parser.add_argument('--imageSet', required=False,
                        metavar="path or URL to imageSet",
                        help='path to imageSet')
    ...
    elif args.command == "rgbDetection":
        rgb_Detect_For_FPointnet(model,args.imageSet,"testing")
